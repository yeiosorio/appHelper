...
import { MyTaskDetailPage } from "../my-task-detail/my-task-detail";
...
export class MyTaskPage {
  ...
  public getListTask() {
    ...
    this.serviceUser.myListTask(this.uid, this.idsubcategories)
      .map(data => data.json())
      .subscribe(data => {
        if (data) {
  ...
  taskDetail(requestId) {
    this.navCtrl.push(MyTaskDetailPage, { requestId: requestId });
  }
